 ...
 * */
inline void EncodeTimeNow(ULID &ulid) { EncodeTime(std::time(nullptr), ulid); }
 ...
 * */
inline void EncodeEntropy(std::function<uint8_t()> const &rng, ULID &ulid) {
  ulid = (ulid >> 80) << 80;
 ...
 * */
inline void Encode(time_t timestamp, std::function<uint8_t()> const &rng, ULID &ulid) {
 ...
 * */
inline ULID Create(time_t timestamp, std::function<uint8_t()> const &rng) {
  ULID ulid = 0;
 ...
 * Crockford's Base32
 * */
static char const Encoding[33] = "0123456789ABCDEFGHJKMNPQRSTVWXYZ";
 ...
 * */
inline void UnmarshalFrom(char const str[26], ULID &ulid) {
  // timestamp
 ...
 * */
inline ULID Unmarshal(std::string const &str) {
  ULID ulid;
 ...
 * */
inline void UnmarshalBinaryFrom(uint8_t const b[16], ULID &ulid) {
  // timestamp
  ulid = b[0];
 ...
 * */
inline ULID UnmarshalBinary(std::vector<uint8_t> const &b) {
  ULID ulid;
